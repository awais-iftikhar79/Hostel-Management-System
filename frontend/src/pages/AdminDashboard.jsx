...
export default function AdminDashboard() {
  ...
  const [selectedHostel, setSelectedHostel] = useState(localStorage.getItem('selectedHostelId') || 'all');
  ...
  const handleDeleteHostel = async () => {
    if (!selectedHostel || selectedHostel === 'all') return;
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
               {hostels.length === 0 ? (
                 <div className="col-span-full text-center py-12 bg-white rounded-xl border border-dashed border-slate-300">
                    <span className="material-symbols-outlined text-4xl text-slate-400 mb-2">domain_disabled</span>
                    <p className="text-slate-600 font-medium text-lg">No Hostels Found</p>
                    <p className="text-slate-500 text-sm">Use the "+ Add Hostel" button in the top right to get started.</p>
                 </div>
               ) : (
                 hostels.map(hostel => (
                   <div 
                   ...
                   >
                     <div className="flex items-center gap-4 mb-4">
                        ...
                        <div>
                          <h3 className="font-bold text-lg text-slate-900">{hostel.name}</h3>
                          <p className="text-sm font-medium text-slate-500">ID: {hostel.id}</p>
                        </div>
                     </div>
                     <div className="flex justify-between items-center border-t border-slate-100 pt-4">
                        <span className="text-sm font-semibold text-slate-600">View Room Map</span>
