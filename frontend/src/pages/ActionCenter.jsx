  ...
  const handleGenerateBills = async (e) => {
    ...
    try {
      ...
      if(res.ok) {
        e.target.reset();
        fetchData();
        alert("Bills successfully generated and divided among active room members!");
      }
    } catch(err) {
      console.error(err);
    }
        ...
        {activeTab === 'payments' && (
          <div className="flex flex-col gap-8">
            ...
            <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-6">
              ...
              <form onSubmit={handleGenerateBills} className="grid grid-cols-1 md:grid-cols-4 gap-4 items-end">
                <div>
                ...
                <div>
                  ...
                  <select name="room_id" required className="w-full rounded-md border border-slate-300 px-3 py-2 text-sm focus:ring-2 focus:ring-blue-500 outline-none bg-white">
                    <option value="all">All Active Rooms</option>
                  </select>
                </div>
                <button type="submit" className="bg-slate-900 text-white font-medium text-sm px-4 py-2 rounded-md hover:bg-slate-800 transition-colors">
            ...
            <div className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
              <div className="px-6 py-4 border-b border-slate-200 flex justify-between items-center bg-slate-50">
                ...
                <select value={paymentStatusFilter} onChange={(e) => setPaymentStatusFilter(e.target.value)} className="rounded-md border border-slate-300 px-4 py-2 bg-white text-sm">
                  <option value="All">All Statuses</option>
                    ...
                    {filteredPayments.map((payment) => (
                      <tr key={payment.id} className="hover:bg-slate-50">
                        <td className="py-4 px-6">
                            ...
                            <p className="text-xs text-slate-500">{payment.student_id_str}</p>
                        </td>
